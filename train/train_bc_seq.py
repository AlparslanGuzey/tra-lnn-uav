# train/train_bc_seq.py
import argparse, json, os
import numpy as np
import torch
import torch.nn as nn
from torch.utils.data import TensorDataset, DataLoader
from lnn.liquid import LiquidPolicySeq

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--data', type=str, default='results/imitation_seq_T8.npz')
    ap.add_argument('--epochs', type=int, default=20)
    ap.add_argument('--batch_size', type=int, default=256)
    ap.add_argument('--lr', type=float, default=3e-4)
    ap.add_argument('--seed', type=int, default=42)
    ap.add_argument('--save', type=str, default='results/bc_lnn_seq.pt')
    args = ap.parse_args()
    ...
    in_dim = X.shape[2]
    model = LiquidPolicySeq(in_dim=in_dim)

    # class weights (handle action imbalance)
    counts = np.bincount(y, minlength=9).astype(np.float32)
    inv = 1.0 / np.maximum(counts, 1.0)
    weights = torch.tensor(inv / inv.sum() * 9.0, dtype=torch.float32)  # normalize
    ...
    with open('results/train_seq_summary.json','w') as f:
